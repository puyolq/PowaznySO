...
void PCB::ustawStatus(int _status)
{
	if (_status>0 && _status<5)
	{
		...
		//zmiana z aktywnego na zako�czony
		else if (status == 3 && _status == 4)
		{
			status = _status;
			usunProces(this->dajNazwe()); //DAMIAN: skutkuje tym �e nigdy prcoes nie jest zako�czony, PCB si� zeruje, dane wskazuj�ce s� losowe
		}
